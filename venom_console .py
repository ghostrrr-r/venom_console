...
# ---- AI Configuration ----
# Shared API key for all users (encoded for basic obfuscation)
import base64
...
HF_API_KEY = base64.b64decode(b64_string).decode('utf-8')
...
def cmd_chatbot(self=None, arg=None):
    ...
    Uses shared API key included with the project.
    """
    ...
    if not api_key or not api_key.startswith("hf_"):
        print("API key configuration error. Please check the code.")
        return
...
def cmd_get_ip(args=""):
    """Resolve domain/website/email to IP address(es)."""
    target = args.strip() or input("Domain, website, or email to resolve: ").strip()
    if not target:
        return
